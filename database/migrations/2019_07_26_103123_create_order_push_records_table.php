...
class CreateOrderPushRecordsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('order_push_records', function (Blueprint $table) {
            ...
            $table->string('extra', 512)->default('')->comment('额外信息，如资金进件返回信息');
            ...
            $table->dateTime('request_time')->nullable()->comment('进件发起时间');
            $table->dateTime('finish_time')->nullable()->comment('进件完成时间');
            $table->integer('status')->default(0)->comment('状态, -1:失败，0:初始化，1:成功');
            $table->timestamps();
            $table->index(['procedure_id', 'order_id'], 'po');
    ...
    {
        Schema::drop('order_push_records');
    }
